"use client";

import React, { useCallback, useState } from "react";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { useSidebar } from "@/context/SidebarContext";
import {
  GridIcon,
  GroupIcon,
  UserIcon,
  UserCircleIcon,
  ChatIcon,
  ChevronDownIcon,
  TaskIcon,
  ListIcon,
  DollarLineIcon,
  BellIcon,
} from "@/icons/index";
import { useAuth } from "@/context/AuthContext";

type SubSubItem = {
  name: string;
  path: string;
};

type SubItem = {
  name: string;
  path: string;
  icon?: React.ReactNode;
  sub_sub_items?: SubSubItem[];
};

type SidebarItem = {
  name: string;
  icon: React.ReactNode;
  path: string;
  highlight?: boolean;
  sub_items?: SubItem[];
  permission?: string;
  is_group?: boolean;
};

type SidebarSection = {
  title?: string;
  items: SidebarItem[];
};

const admin_sidebar_sections: SidebarSection[] = [
  {
    items: [
      {
        name: "Dashboard",
        icon: <GridIcon />,
        path: "/admin/dashboard",
        highlight: true,
      },
    ],
  },
  {
    title: "MANAGEMENT",
    items: [
      {
        name: "Users",
        icon: <GroupIcon />,
        path: "/admin/users",
        permission: "users.view",
      },
      {
        name: "Clients",
        icon: <UserIcon />,
        path: "#",
        is_group: true,
        permission: "users.view",
        sub_items: [
          {
            name: "All Clients",
            path: "/admin/clients",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
              </svg>
            ),
          },
          {
            name: "Client Invitations",
            path: "/admin/clients/invitations",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
              </svg>
            ),
          },
          {
            name: "Welcome Emails",
            path: "/admin/clients/welcome-emails",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5" />
              </svg>
            ),
          },
        ],
      },
      {
        name: "Organizations",
        icon: <TaskIcon />,
        path: "/admin/organizations",
        permission: "organizations.view",
      },
      {
        name: "Orders",
        icon: <ListIcon />,
        path: "/admin/orders",
        permission: "orders.view",
      },
      {
        name: "Tracking",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
          </svg>
        ),
        path: "/admin/tracking",
        permission: "orders.view",
      },
      {
        name: "Scheduled Calls",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5m-9-6h.008v.008H12v-.008zM12 15h.008v.008H12V15zm0 2.25h.008v.008H12v-.008zM9.75 15h.008v.008H9.75V15zm0 2.25h.008v.008H9.75v-.008zM7.5 15h.008v.008H7.5V15zm0 2.25h.008v.008H7.5v-.008zm6.75-4.5h.008v.008h-.008v-.008zm0 2.25h.008v.008h-.008V15zm0 2.25h.008v.008h-.008v-.008zm2.25-4.5h.008v.008H16.5v-.008zm0 2.25h.008v.008H16.5V15z" />
          </svg>
        ),
        path: "/admin/scheduled-calls",
        permission: "orders.view",
      },
      {
        name: "Invoices",
        icon: <DollarLineIcon />,
        path: "/admin/invoices",
        permission: "invoices.view",
      },
      {
        name: "Transactions",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
          </svg>
        ),
        path: "/admin/transactions",
        permission: "invoices.view",
      },
      {
        name: "Credits",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
          </svg>
        ),
        path: "#",
        is_group: true,
        permission: "invoices.view",
        sub_items: [
          {
            name: "Clients Credits",
            path: "/admin/credits/clients",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
              </svg>
            ),
          },
          {
            name: "Credit Purchases",
            path: "/admin/credits/purchases",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
              </svg>
            ),
          },
          {
            name: "Assign Credits",
            path: "/admin/credits",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
              </svg>
            ),
          },
        ],
      },
      {
        name: "Services",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" />
          </svg>
        ),
        path: "#",
        is_group: true,
        permission: "orders.view",
        sub_items: [
          {
            name: "Link Building",
            path: "/admin/link-building",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
              </svg>
            ),
          },
          {
            name: "Premium Mentions",
            path: "#",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
              </svg>
            ),
            sub_sub_items: [
              { name: "Plans", path: "/admin/premium-mentions" },
              { name: "Appointments", path: "/admin/premium-mentions/appointments" },
            ],
          },
          {
            name: "SME Content",
            path: "/admin/sme-content",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z" />
              </svg>
            ),
            sub_sub_items: [
              { name: "Authored", path: "/admin/sme-content/authored" },
              { name: "Collaboration", path: "/admin/sme-content/collaboration" },
              { name: "Enhanced", path: "/admin/sme-content/enhanced" },
              { name: "Appointments", path: "/admin/sme-appointments" },
            ],
          },
          {
            name: "New Content",
            path: "/admin/new-content",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
              </svg>
            ),
          },
          {
            name: "Content Refresh",
            path: "/admin/content-refresh",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
              </svg>
            ),
            sub_sub_items: [
              { name: "Content Optimization", path: "/admin/content-optimization" },
              { name: "Content Briefs", path: "/admin/content-briefs" },
            ],
          },
          {
            name: "SEO Packages",
            path: "#",
            icon: (
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            ),
            sub_sub_items: [
              { name: "Packages", path: "/admin/seo-packages" },
              { name: "Subscriptions", path: "/admin/seo-packages-subscriptions" },
              { name: "Appointments", path: "/admin/seo-packages-appointments" },
            ],
          },
        ],
      },
      {
        name: "Coupons",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 14.25l6-6m4.5-3.493V21.75l-3.75-1.5-3.75 1.5-3.75-1.5-3.75 1.5V4.757c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0c1.1.128 1.907 1.077 1.907 2.185zM9.75 9h.008v.008H9.75V9zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm4.125 4.5h.008v.008h-.008V13.5zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
          </svg>
        ),
        path: "/admin/coupons",
        permission: "orders.view",
      },
      {
        name: "Discounts",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 14.25l6-6m4.5-3.493V21.75l-3.75-1.5-3.75 1.5-3.75-1.5-3.75 1.5V4.757c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0c1.1.128 1.907 1.077 1.907 2.185z" />
            <circle cx="9.75" cy="9" r="0.375" fill="currentColor" />
            <circle cx="14.25" cy="13.5" r="0.375" fill="currentColor" />
          </svg>
        ),
        path: "/admin/discounts",
        permission: "orders.view",
      },
      {
        name: "News & Promos",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M10.34 15.84c-.688-.06-1.386-.09-2.09-.09H7.5a4.5 4.5 0 110-9h.75c.704 0 1.402-.03 2.09-.09m0 9.18c.253.962.584 1.892.985 2.783.247.55.06 1.21-.463 1.511l-.657.38c-.551.318-1.26.117-1.527-.461a20.845 20.845 0 01-1.44-4.282m3.102.069a18.03 18.03 0 01-.59-4.59c0-1.586.205-3.124.59-4.59m0 9.18a23.848 23.848 0 018.835 2.535M10.34 6.66a23.847 23.847 0 008.835-2.535m0 0A23.74 23.74 0 0018.795 3m.38 1.125a23.91 23.91 0 011.014 5.395m-1.014 8.855c-.118.38-.245.754-.38 1.125m.38-1.125a23.91 23.91 0 001.014-5.395m0-3.46c.495.413.811 1.035.811 1.73 0 .695-.316 1.317-.811 1.73m0-3.46a24.347 24.347 0 010 3.46" />
          </svg>
        ),
        path: "/admin/news",
        permission: "orders.view",
      },
      {
        name: "Resources",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
          </svg>
        ),
        path: "/admin/resources",
        permission: "resources.view",
      },
      {
        name: "News Placements",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418" />
          </svg>
        ),
        path: "/admin/news-placements",
        permission: "orders.view",
      },
      {
        name: "Support Tickets",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 6v.75m0 3v.75m0 3v.75m0 3V18m-9-5.25h5.25M7.5 15h3M3.375 5.25c-.621 0-1.125.504-1.125 1.125v3.026a2.999 2.999 0 0 1 0 5.198v3.026c0 .621.504 1.125 1.125 1.125h17.25c.621 0 1.125-.504 1.125-1.125v-3.026a2.999 2.999 0 0 1 0-5.198V6.375c0-.621-.504-1.125-1.125-1.125H3.375Z" />
          </svg>
        ),
        path: "/admin/support-tickets",
        permission: "orders.view",
      },
    ],
  },
  {
    title: "TEAM",
    items: [
      {
        name: "Teams",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z" />
          </svg>
        ),
        path: "/admin/teams",
        permission: "invitations.manage",
      },
      {
        name: "Invitations",
        icon: <ChatIcon />,
        path: "/admin/invitations",
        permission: "invitations.manage",
      },
    ],
  },
  {
    title: "ACCOUNT",
    items: [
      { name: "Notifications", icon: <BellIcon />, path: "/admin/notifications" },
      { name: "Profile", icon: <UserCircleIcon />, path: "/admin/profile" },
      {
        name: "Email Settings",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
          </svg>
        ),
        path: "/admin/email-notifications",
      },
    ],
  },
];

const AdminSidebar: React.FC = () => {
  const { isExpanded, isMobileOpen, isHovered, setIsHovered } = useSidebar();
  const { hasPermission } = useAuth();
  const pathname = usePathname();
  const [expanded_menus, setExpandedMenus] = useState<Record<string, boolean>>({});

  const isActive = useCallback(
    (path: string) => path === pathname || pathname.startsWith(path + "/"),
    [pathname]
  );

  const isSubSubActive = useCallback(
    (sub_sub_items: SubSubItem[]) => sub_sub_items.some((ssi) => isActive(ssi.path)),
    [isActive]
  );

  const isSubItemActive = useCallback(
    (sub_item: SubItem) =>
      isActive(sub_item.path) || isSubSubActive(sub_item.sub_sub_items ?? []),
    [isActive, isSubSubActive]
  );

  const isGroupActive = useCallback(
    (sub_items: SubItem[]) => sub_items.some(isSubItemActive),
    [isSubItemActive]
  );

  const toggleMenu = useCallback((key: string, current_open: boolean) => {
    setExpandedMenus((prev) => ({ ...prev, [key]: !current_open }));
  }, []);

  const showText = isExpanded || isHovered || isMobileOpen;

  const getVisibleItems = useCallback(
    (items: SidebarItem[]) =>
      items.filter((item) =>
        item.permission ? hasPermission(item.permission) : true
      ),
    [hasPermission]
  );

  const visible_sections = admin_sidebar_sections.filter(
    (section) => getVisibleItems(section.items).length > 0
  );

  return (
    <aside
      className={`fixed mt-16 flex flex-col lg:mt-0 top-0 px-5 left-0 bg-gray-900 dark:bg-gray-950 text-white h-screen transition-all duration-300 ease-in-out z-50 border-r border-gray-700 dark:border-gray-800
        ${isExpanded || isMobileOpen
          ? "w-[290px]"
          : isHovered
            ? "w-[290px]"
            : "w-[90px]"
        }
        ${isMobileOpen ? "translate-x-0" : "-translate-x-full"}
        lg:translate-x-0`}
      onMouseEnter={() => !isExpanded && setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Logo */}
      <div
        className={`py-8 flex ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}
      >
        <Link href="/admin/dashboard" className="flex items-center gap-2">
          {showText ? (
            <div className="flex flex-col">
              <span className="text-xl font-bold text-white">
                BASE{" "}
                <span className="text-sm font-normal text-gray-400">
                  Search Marketing
                </span>
              </span>
              <span className="text-xs font-medium text-brand-400 mt-0.5 tracking-wider uppercase">
                Admin Portal
              </span>
            </div>
          ) : (
            <span className="text-xl font-bold text-white">B</span>
          )}
        </Link>
      </div>

      {/* Navigation */}
      <div className="flex flex-col overflow-y-auto duration-300 ease-linear no-scrollbar">
        <nav className="mb-6">
          <div className="flex flex-col gap-4">
            {visible_sections.map((section, section_index) => (
              <div key={section_index}>
                {section.title && showText && (
                  <h2 className="mb-4 text-xs uppercase leading-[20px] text-gray-500">
                    {section.title}
                  </h2>
                )}
                {section.title && !showText && (
                  <div className="mb-4 flex justify-center">
                    <div className="h-px w-6 bg-gray-700" />
                  </div>
                )}

                <ul className="flex flex-col gap-1">
                  {getVisibleItems(section.items).map((item) => {
                    const has_sub_items = item.sub_items && item.sub_items.length > 0;
                    const menu_key = item.name;
                    const group_active = has_sub_items && isGroupActive(item.sub_items!);
                    const is_open = expanded_menus[menu_key] ?? group_active;

                    return (
                      <li key={item.name}>
                        {has_sub_items ? (
                          <>
                            {/* ── Level 1: collapsible group ── */}
                            <button
                              onClick={() => toggleMenu(menu_key, is_open)}
                              className={`relative flex items-center w-full gap-3 px-3 py-2 font-medium rounded-lg text-theme-sm transition-colors ${
                                group_active
                                  ? "bg-brand-500/20 text-brand-400"
                                  : "text-gray-300 hover:bg-white/5 hover:text-white"
                              } ${
                                !isExpanded && !isHovered
                                  ? "lg:justify-center"
                                  : "lg:justify-start"
                              }`}
                            >
                              <span className={group_active ? "text-brand-400" : "text-gray-500"}>
                                {item.icon}
                              </span>
                              {showText && (
                                <>
                                  <span className="flex-1 text-left">{item.name}</span>
                                  <span className={`transition-transform duration-200 ${is_open ? "rotate-180" : ""}`}>
                                    <ChevronDownIcon />
                                  </span>
                                </>
                              )}
                            </button>

                            {/* ── Level 2: sub items ── */}
                            {showText && is_open && (
                              <ul className="mt-1 ml-3 pl-3 flex flex-col gap-0.5 border-l border-gray-700/60">
                                {item.sub_items!.map((sub_item) => {
                                  const has_sub_sub = !!sub_item.sub_sub_items?.length;
                                  const sub_key = `${menu_key}__${sub_item.name}`;
                                  const sub_item_active = isSubItemActive(sub_item);
                                  const is_sub_open = expanded_menus[sub_key] ?? sub_item_active;

                                  return (
                                    <li key={sub_item.name}>
                                      {has_sub_sub ? (
                                        <>
                                          {/* Level 2 with children: button */}
                                          <button
                                            onClick={() => toggleMenu(sub_key, is_sub_open)}
                                            className={`flex items-center w-full gap-2 px-2 py-1.5 text-sm rounded-md transition-colors ${
                                              sub_item_active
                                                ? "text-brand-400 font-medium"
                                                : "text-gray-400 hover:text-gray-200 hover:bg-white/5"
                                            }`}
                                          >
                                            {sub_item.icon && (
                                              <span className={sub_item_active ? "text-brand-400" : "text-gray-600"}>
                                                {sub_item.icon}
                                              </span>
                                            )}
                                            <span className="flex-1 text-left">{sub_item.name}</span>
                                            <span className={`transition-transform duration-200 ${is_sub_open ? "rotate-180" : ""}`}>
                                              <ChevronDownIcon />
                                            </span>
                                          </button>

                                          {/* ── Level 3: sub-sub items ── */}
                                          {is_sub_open && (
                                            <ul className="mt-0.5 ml-3 pl-3 flex flex-col gap-0.5 border-l border-gray-700/40">
                                              {sub_item.sub_sub_items!.map((ssi) => (
                                                <li key={ssi.name}>
                                                  <Link
                                                    href={ssi.path}
                                                    className={`flex items-center gap-2 px-2 py-1 text-xs rounded-md transition-colors ${
                                                      pathname === ssi.path
                                                        ? "text-brand-400 font-medium bg-brand-500/10"
                                                        : "text-gray-500 hover:text-gray-300 hover:bg-white/5"
                                                    }`}
                                                  >
                                                    <span className="w-1 h-1 rounded-full bg-current shrink-0" />
                                                    {ssi.name}
                                                  </Link>
                                                </li>
                                              ))}
                                            </ul>
                                          )}
                                        </>
                                      ) : (
                                        /* Level 2 leaf: direct link */
                                        <Link
                                          href={sub_item.path}
                                          className={`flex items-center gap-2 px-2 py-1.5 text-sm rounded-md transition-colors ${
                                            pathname === sub_item.path
                                              ? "text-brand-400 font-medium bg-brand-500/10"
                                              : "text-gray-400 hover:text-gray-200 hover:bg-white/5"
                                          }`}
                                        >
                                          {sub_item.icon && (
                                            <span className={pathname === sub_item.path ? "text-brand-400" : "text-gray-600"}>
                                              {sub_item.icon}
                                            </span>
                                          )}
                                          {sub_item.name}
                                        </Link>
                                      )}
                                    </li>
                                  );
                                })}
                              </ul>
                            )}
                          </>
                        ) : (
                          /* ── Level 1 leaf: direct link ── */
                          <Link
                            href={item.path}
                            className={`relative flex items-center w-full gap-3 px-3 py-2 font-medium rounded-lg text-theme-sm transition-colors ${
                              item.highlight && isActive(item.path)
                                ? "bg-brand-500 text-white"
                                : isActive(item.path)
                                  ? "bg-white/10 text-white"
                                  : "text-gray-300 hover:bg-white/5 hover:text-white"
                            } ${
                              !isExpanded && !isHovered
                                ? "lg:justify-center"
                                : "lg:justify-start"
                            }`}
                          >
                            <span
                              className={
                                item.highlight && isActive(item.path)
                                  ? "text-white"
                                  : isActive(item.path)
                                    ? "text-brand-400"
                                    : "text-gray-500"
                              }
                            >
                              {item.icon}
                            </span>
                            {showText && <span>{item.name}</span>}
                          </Link>
                        )}
                      </li>
                    );
                  })}
                </ul>
              </div>
            ))}
          </div>
        </nav>
      </div>
    </aside>
  );
};

export default AdminSidebar;
