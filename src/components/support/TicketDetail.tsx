...
const TicketDetail: React.FC<TicketDetailProps> = ({ ticket: initial_ticket, current_user_id }) => {
  ...
  const related_order_label =
    order_options.find((o) => o.value === ticket.related_order)?.label ??
  ...
  const handleSendReply = useCallback(async () => {
    ...
    try {
      const new_message = await supportTicketsService.addMessage(ticket.id, reply.trim());
      setTicket((prev) => ({
        ...prev,
  ...
  const handleStatusUpdate = async (new_status: TicketStatus) => {
    ...
    try {
      ...
      setTicket((prev) => ({ ...prev, status: updated.status, resolved_at: updated.resolved_at, closed_at: updated.closed_at }));
    } finally {
    ...
    <div>
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Conversation thread */}
          <div className="min-h-[280px] max-h-[540px] overflow-y-auto space-y-5 pb-4 pr-1">
            {messages.length === 0 ? (
              ...
              messages.map((msg, i) => {
                const is_own = msg.sender_id === current_user_id;
                ...
                return (
                  <React.Fragment key={msg.id}>
                    ...
                    <MessageBubble message={msg} is_own={is_own} />
                  </React.Fragment>
                );
              })
            )}
            ...
            {is_closed ? (
              <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 px-5 py-6 text-center">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
                  Reopen it to continue the conversation with our team.
                </p>
                <Button
                  variant="outline"
                  size="sm"
                    ...
                    <div className="pb-5 pt-0.5">
                      ...
                      {is_active && step === "resolved" && ticket.resolved_at && (
                        <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">
                          {formatTicketDate(ticket.resolved_at)}
                        </p>
                      )}
...
function MessageBubble({
  ...
  return (
    <div className={`flex gap-3 ${is_own ? "flex-row-reverse" : "flex-row"}`}>
      ...
      {/* Content */}
      <div className={`max-w-[78%] flex flex-col gap-1 ${is_own ? "items-end" : "items-start"}`}>
        {/* Sender row */}
        <div className={`flex items-center gap-1.5 ${is_own ? "flex-row-reverse" : "flex-row"}`}>
...
const action_color_classes: Record<ActionColor, string> = {
  success:
    "text-success-700 border-success-200 hover:bg-success-50 dark:text-success-400 dark:border-success-500/20 dark:hover:bg-success-500/10",
  warning:
    "text-warning-700 border-warning-200 hover:bg-warning-50 dark:text-warning-400 dark:border-warning-500/20 dark:hover:bg-warning-500/10",
  danger:
    "text-error-700 border-error-200 hover:bg-error-50 dark:text-error-400 dark:border-error-500/20 dark:hover:bg-error-500/10",
  brand:
    "text-brand-700 border-brand-200 hover:bg-brand-50 dark:text-brand-400 dark:border-brand-500/20 dark:hover:bg-brand-500/10",
};
