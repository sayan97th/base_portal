...
interface UnderlineFieldProps {
  ...
  error?: string;
  right_adornment?: React.ReactNode;
}
  ...
  <div>
    <div className="relative">
      <input
        ...
        placeholder={label}
        className={`peer w-full border-0 border-b bg-transparent px-0.5 pb-2 pt-4 text-sm text-gray-900 placeholder-transparent focus:border-brand-500 focus:outline-none focus:ring-0 dark:text-white ${
          error ? "border-error-400" : "border-gray-300 dark:border-gray-700"
        }`}
      />
      <label className="pointer-events-none absolute left-0.5 top-4 text-sm text-gray-400 transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-sm peer-focus:-top-0.5 peer-focus:text-xs peer-focus:text-brand-500 peer-[:not(:placeholder-shown)]:-top-0.5 peer-[:not(:placeholder-shown)]:text-xs dark:text-gray-500">
        {label}
      </label>
      {right_adornment && (
        <span className="absolute right-1 top-1/2 -translate-y-1/2">{right_adornment}</span>
      )}
    </div>
...
const PublicAccountStep: React.FC<PublicAccountStepProps> = ({
  ...
  const [is_google_loading, setIsGoogleLoading] = useState(false);

  // Already signed in: skip this step entirely and move straight to payment.
  useEffect(() => {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(api_error.message || "An error occurred while creating your account.");
    } finally {
  ...
  return (
    <div>
      <button
        type="button"
        onClick={onBack}
        className="mb-6 inline-flex items-center gap-1.5 text-sm font-medium text-gray-400 transition-colors hover:text-gray-700 dark:text-gray-500 dark:hover:text-gray-300"
      >
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        ...
        <button
          type="button"
          disabled
          title="Facebook login is not available yet"
          className="inline-flex cursor-not-allowed items-center justify-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm font-medium text-gray-400 dark:border-gray-800 dark:bg-white/5 dark:text-gray-600"
        >
      ...
      <div className="relative my-6">
        ...
        <div className="relative flex justify-center text-xs font-medium tracking-wide text-gray-400">
          <span className="bg-white px-3 dark:bg-gray-900">OR</span>
        </div>
      </div>

      {error && (
        <div className="mb-5 rounded-lg bg-error-50 p-3 text-sm text-error-600 dark:bg-error-500/10 dark:text-error-400">
          {error}
        </div>
      )}
...
export default PublicAccountStep;
