...
function countUnanswered(comments: OrderComment[]): number {
  return comments.reduce((acc, c) => {
    const top_unanswered =
      !c.is_admin_comment && (c.replies ?? []).every((r) => !r.is_admin_comment)
        ? 1
        : 0;
    return acc + top_unanswered + countUnanswered(c.replies ?? []);
  }, 0);
}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 max-w-xs">
        Client messages and your staff replies will appear here. Use the form below to proactively reach out.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="mt-2 space-y-2">
      ...
      <div className="flex items-center justify-between">
        <span className={`text-[11px] ${chars_remaining < 50 ? "text-warning-500" : "text-gray-400 dark:text-gray-500"}`}>
  ...
  return (
    <div className="mt-3 ml-11 flex gap-2.5">
      ...
      <div className="flex-1 space-y-2">
        <div className="rounded-lg border border-violet-200 dark:border-violet-500/30 bg-violet-50/50 dark:bg-violet-500/5 p-1">
          ...
          <textarea
            ...
            rows={2}
            maxLength={MAX_CHARS}
            placeholder="Write your staff reply to the client..."
            className="w-full resize-none rounded-md border border-violet-200 dark:border-violet-500/20 bg-white dark:bg-gray-900 px-3 py-2 text-sm text-gray-800 dark:text-white/90 placeholder:text-gray-400 focus:border-violet-400 focus:outline-none focus:ring-1 focus:ring-violet-400/30 transition-colors"
          />
        </div>
        <div className="flex items-center justify-between">
          <span className={`text-[11px] ${chars_remaining < 50 ? "text-warning-500" : "text-gray-400 dark:text-gray-500"}`}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-md bg-violet-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
            >
              {is_submitting ? (
                <Spinner className="h-3 w-3 rounded-full border-2 border-white/30 border-t-white animate-spin" />
              ) : (
...
function AdminCommentItem({
  ...
  const needs_reply =
    ...
    (comment.replies ?? []).every((r) => !r.is_admin_comment);
      ...
      <div
        className={`flex gap-3 ${
          comment.is_admin_comment
            ...
            : needs_reply
            ? "rounded-xl bg-amber-50/60 dark:bg-amber-500/5 px-3 py-2.5 border border-amber-100 dark:border-amber-500/20"
            : ""
        }`}
      >
        <CommentAvatar
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-1.5 mb-1">
            ...
            {comment.is_admin_comment ? (
              ...
              <span className="inline-flex items-center rounded-full bg-amber-100 dark:bg-amber-500/20 px-1.5 py-0.5 text-[10px] font-bold tracking-wide text-amber-700 dark:text-amber-300 uppercase">
                Client
              </span>
            )}

            {needs_reply && (
              <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 dark:bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-semibold text-amber-600 dark:text-amber-400">
              ...
              {!is_reply && (
                <button
                  ...
                  className={`inline-flex items-center gap-1 text-[11px] font-medium transition-colors ${
                    needs_reply
                      ? "text-violet-500 hover:text-violet-700 dark:text-violet-400 dark:hover:text-violet-300 font-semibold"
                      : "text-gray-400 hover:text-violet-500 dark:hover:text-violet-400"
                  }`}
                >
                  <StaffReplyIcon />
                  {needs_reply ? "Reply to Client" : "Reply"}
                </button>
              )}
              {is_own && (
                <button
...
const AdminOrderComments: React.FC<AdminOrderCommentsProps> = ({
  ...
  const unanswered_count = countUnanswered(comments);
  ...
  const handleEdit = useCallback(async (comment_id: number, content: string) => {
    const updated = await adminOrderCommentsService.updateComment(comment_id, {
      content,
    });
  ...
  const handleDelete = useCallback(async (comment_id: number) => {
    await adminOrderCommentsService.deleteComment(comment_id);
    ...
    <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/3 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-50 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400">
            <ChatBubbleIcon />
          </div>
          <div>
        ...
        <div className="flex items-center gap-2">
          {!is_loading && unanswered_count > 0 && (
            <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 text-xs font-semibold text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/30">
              <span className="h-1.5 w-1.5 rounded-full bg-amber-500 animate-pulse" />
              {unanswered_count} awaiting reply
            </span>
          )}
          {!is_loading && !load_error && total_count > 0 && unanswered_count === 0 && (
            <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30">
              <CheckIcon />
              All replied
            </span>
          )}
      ...
      <div className="px-5 sm:px-6 py-5">
        {is_loading ? (
          ...
          <div className="rounded-xl border border-error-100 dark:border-error-500/20 bg-error-50 dark:bg-error-500/10 px-4 py-3.5">
            <p className="text-sm text-error-700 dark:text-error-400">{load_error}</p>
          </div>
        ) : comments.length === 0 ? (
          <EmptyState />
        ) : (
          <div className="space-y-5 divide-y divide-gray-100 dark:divide-gray-800">
            {comments.map((comment, index) => (
              <div key={comment.id} className={index > 0 ? "pt-5" : ""}>
                <AdminCommentItem
                  comment={comment}
                  current_user_id={current_user_id}
                  current_user_name={current_user_name}
      ...
      <div className="px-5 sm:px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-violet-50/30 dark:bg-violet-500/5">
        <div className="mb-3 flex items-center gap-2">
          <span className="h-1.5 w-1.5 rounded-full bg-violet-500" />
          <p className="text-xs font-bold uppercase tracking-wide text-violet-600 dark:text-violet-400">
            Send Staff Message to Client
          </p>
        </div>
          ...
          <div className="flex-1 space-y-2.5">
            <div className="relative">
              <textarea
                ...
                rows={3}
                maxLength={MAX_CHARS}
                placeholder="Write a message to the client… (Ctrl+Enter to send)"
                className="w-full resize-none rounded-xl border border-violet-200 dark:border-violet-500/30 bg-white dark:bg-gray-900 px-3.5 py-3 text-sm text-gray-800 dark:text-white/90 placeholder:text-gray-400 dark:placeholder:text-gray-600 focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-400/20 transition-colors shadow-xs"
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              <span
                className={`text-[11px] ${
                  chars_remaining < 100 ? "text-warning-500" : "text-gray-400 dark:text-gray-500"
                }`}
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-violet-600 px-4 py-2 text-xs font-semibold text-white hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
              >
                {is_submitting ? <Spinner /> : <SendIcon />}
                Send to Client
              </button>
            </div>
          </div>
...
export default AdminOrderComments;
