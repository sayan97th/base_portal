...
export default function ReportTableCard({
  ...
  // Inline add row
  const [is_adding_inline, setIsAddingInline] = useState(false);
  const [is_saving_inline, setIsSavingInline] = useState(false);

  // Inline edit row
  const [inline_editing_row_id, setInlineEditingRowId] = useState<string | null>(null);
  ...
  async function handleSaveEdit(row_id: string, payload: UpdateReportRowPayload) {
    setIsSavingEdit(true);
    try {
      await onEditRow(table.id, row_id, payload);
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  ...
                  <th className="whitespace-nowrap px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                    Exact Match
                  </th>
                  <th className="whitespace-nowrap px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                    Request Date
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {/* Empty state — only when no rows and not adding inline */}
                {rows_count === 0 && !is_adding_inline && (
                  <tr>
                ...
                {/* Existing rows */}
                {table.rows.map((row) => {
                  // Render inline editor when this row is being edited
                  if (inline_editing_row_id === row.id) {
                    return (
                      <InlineEditRow
                        key={row.id}
                        row={row}
                        is_saving={is_saving_edit}
                        onSave={(payload) => handleSaveEdit(row.id, payload)}
                        onCancel={() => setInlineEditingRowId(null)}
                      />
                    );
                  }
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs font-medium text-gray-800 dark:text-white/90">
                        {row.keyword || <span className="font-normal text-gray-300 dark:text-gray-600">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        {row.landing_page ? (
                          <a
                      ...
                      <td className="px-4 py-3 text-right">
                        {is_confirming_delete ? (
                          ...
                          <div className="flex items-center justify-end gap-1">
                            <button
                              onClick={() => setInlineEditingRowId(row.id)}
                ...
                {/* Inline add row — always at the bottom when active */}
                {is_adding_inline && (
                  <InlineAddRow
                    order_id={order_id}
                    is_saving={is_saving_inline}
                    onSave={handleAddRow}
