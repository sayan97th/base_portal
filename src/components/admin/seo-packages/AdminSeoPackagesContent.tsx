...
import {
  ...
  createAdminSeoPackage,
  updateAdminSeoPackage,
  toggleAdminSeoPackageStatus,
  deleteAdminSeoPackage,
      ...
      <div className="relative w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-gray-900">
        <div className="p-6">
          ...
          <h3 className="mt-4 text-base font-semibold text-gray-900 dark:text-white">
            Delete &ldquo;{package_data.name}&rdquo;?
          </h3>
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            This action cannot be undone. All data associated with this package will be permanently
            removed.
          </p>
        </div>
...
export default function AdminSeoPackagesContent() {
  ...
  const [deleting_package, setDeletingPackage] = useState<AdminSeoPackage | null>(null);
  ...
  const fetchPackages = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await listAdminSeoPackages();
      setPackages(data);
    } catch {
  ...
  const active_count = packages.filter((p) => p.is_active).length;
  const disabled_count = packages.filter((p) => !p.is_active).length;

  // ── Filter ─────────────────────────────────────────────────────────────────

  const filtered_packages = packages
    .filter((p) => {
  ...
  const handleCreate = async (payload: CreateSeoPackagePayload) => {
    await createAdminSeoPackage(payload);
    fetchPackages();
  };

  const handleUpdate = async (payload: CreateSeoPackagePayload) => {
    if (!editing_package) return;
    await updateAdminSeoPackage(editing_package.id, payload as UpdateSeoPackagePayload);
    fetchPackages();
  };

  const handleToggleStatus = async (pkg: AdminSeoPackage, is_active: boolean) => {
    await toggleAdminSeoPackageStatus(pkg.id, is_active);
    fetchPackages();
  };

  const handleDelete = async () => {
    if (!deleting_package) return;
    setIsDeleting(true);
    try {
      await deleteAdminSeoPackage(deleting_package.id);
  ...
  const openEdit = (pkg: AdminSeoPackage) => {
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by name, slug, tagline, or best for..."
        ...
        {filtered_packages.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {filtered_packages.map((pkg) => (
              <SeoPackageCard
                key={pkg.id}
                package_data={pkg}
                onEdit={() => openEdit(pkg)}
                onDelete={() => setDeletingPackage(pkg)}
                onToggleStatus={(is_active) => handleToggleStatus(pkg, is_active)}
              />
            ))}
          </div>
        )}
      ...
      <SeoPackageFormModal
        ...
        package_data={editing_package}
        onClose={() => {
          setFormModalOpen(false);
          setEditingPackage(null);
        }}
        onSubmit={editing_package ? handleUpdate : handleCreate}
      />

      {/* Delete confirmation modal */}
      <DeleteConfirmModal
        package_data={deleting_package}
