...
import { adminSeoPackagesAppointmentService } from "@/services/admin/seo-packages-appointment.service";
import type {
  AdminSeoAppointment,
  AdminSeoAppointmentFilters,
  SeoAppointmentStatus,
  SeoAppointmentSortField,
  SortDirection,
} from "@/services/admin/seo-packages-appointment.service";
...
const SORTABLE_COLS: { field: SeoAppointmentSortField; label: string }[] = [
...
function SortIcon({
  ...
  field: SeoAppointmentSortField;
  active_field?: SeoAppointmentSortField;
...
export default function AdminSeoPackagesAppointmentsContent() {
  const [appointments, setAppointments] = useState<AdminSeoAppointment[]>([]);
  ...
  const [status_filter, setStatusFilter] = useState<SeoAppointmentStatus | "">("");
  const [sort_field, setSortField] = useState<SeoAppointmentSortField>("scheduled_at");
  ...
  const fetchAppointments = useCallback(async (filters: AdminSeoAppointmentFilters) => {
    setIsLoading(true);
    setError(null);
    try {
      const data = await adminSeoPackagesAppointmentService.fetchAppointments(filters);
  ...
  function handleStatusFilter(value: SeoAppointmentStatus | "") {
    setStatusFilter(value);
    setPage(1);
  }

  function handleColumnSort(field: SeoAppointmentSortField) {
    if (sort_field === field) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">
            SEO Packages Appointments
          </h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {total > 0 ? `${total} total appointments` : "Manage all SEO Packages appointments"}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          value={status_filter}
          onChange={(e) => handleStatusFilter(e.target.value as SeoAppointmentStatus | "")}
                        ...
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-xs text-gray-400 dark:text-gray-500">
                            {has_active_filters
                              ...
                              : "Appointments will appear here once clients schedule SEO package sessions."}
                          </p>
                          {has_active_filters && (
                            <button
                              onClick={handleClearFilters}
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-4">
                          <Link href={`/admin/seo-packages-appointments/${apt.id}`} className="block">
                        ...
                        <td className="px-5 py-4">
                          <Link href={`/admin/seo-packages-appointments/${apt.id}`} className="block">
                            <p className="font-medium text-gray-900 dark:text-white">
                              {apt.package?.name ?? <span className="text-gray-400">—</span>}
                            </p>
                            {apt.package?.price_per_month != null && (
                              <p className="text-xs text-gray-400">
                                ${Number(apt.package.price_per_month).toLocaleString()}/mo
                              </p>
                            )}
                          </Link>
                        </td>
                        ...
                        <td className="px-5 py-4 text-right">
                          <Link
                            href={`/admin/seo-packages-appointments/${apt.id}`}
