...
const STATUS_TRANSITIONS: Record<SeoAppointmentStatus, SeoAppointmentStatus[]> = {
...
function SkeletonLoader() {
  return (
    <div className="space-y-6">
      <Link href="/admin/seo-packages-appointments" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
...
interface UpdateModalProps {
  appointment: AdminSeoAppointment;
  modal_status: SeoAppointmentStatus | "";
  modal_scheduled_at: string;
  modal_admin_notes: string;
  is_saving: boolean;
  onStatusChange: (s: SeoAppointmentStatus | "") => void;
...
interface AdminSeoPackagesAppointmentDetailProps {
  appointment_id: number;
}

export default function AdminSeoPackagesAppointmentDetail({
  appointment_id,
}: AdminSeoPackagesAppointmentDetailProps) {
  const router = useRouter();
  const [appointment, setAppointment] = useState<AdminSeoAppointment | null>(null);
  const [is_loading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [show_update_modal, setShowUpdateModal] = useState(false);
  const [modal_status, setModalStatus] = useState<SeoAppointmentStatus | "">("");
  ...
  const fetchAppointment = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await adminSeoPackagesAppointmentService.fetchAppointmentById(appointment_id);
      setAppointment(data);
    } catch {
  ...
  async function handleUpdateConfirm() {
    ...
    setIsSaving(true);
    try {
      const payload: UpdateSeoAppointmentPayload = {};
      ...
      const updated = await adminSeoPackagesAppointmentService.updateAppointment(
        appointment.id,
        payload
      );
  ...
  async function handleDeleteConfirm() {
    ...
    setIsDeleting(true);
    try {
      await adminSeoPackagesAppointmentService.deleteAppointment(appointment.id);
      router.push("/admin/seo-packages-appointments");
    } catch {
  ...
  if (error && !appointment) {
    return (
      <div className="space-y-4">
        <Link
          href="/admin/seo-packages-appointments"
                ...
                <div className="flex items-center justify-between px-5 py-4">
                  <div>
                    ...
                    <p className="mt-0.5 font-mono text-xs text-gray-400">
                      {appointment.package.slug}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold text-gray-900 dark:text-white">
                      ${Number(appointment.package.price_per_month).toLocaleString()}
                    </p>
