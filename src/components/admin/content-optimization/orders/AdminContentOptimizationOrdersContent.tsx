...
import { listAdminContentOptimizationOrders } from "@/services/admin/content-optimization.service";
import type { AdminOrder, OrderStatus, PaginatedResponse } from "@/types/admin";
import type { AdminContentOptimizationOrderFilters } from "@/types/admin/content-optimization";
    ...
    <div className="relative">
      <input
        ...
        className={`h-9 w-full cursor-pointer rounded-xl border pl-9 pr-3 text-sm transition-colors focus:outline-none focus:ring-3 ${
          value
            ? "border-violet-300 bg-violet-50 text-violet-700 focus:border-violet-400 focus:ring-violet-500/10 dark:border-violet-700 dark:bg-violet-500/10 dark:text-violet-300"
            : "border-gray-200 bg-white text-gray-700 placeholder-gray-400 focus:border-violet-400 focus:ring-violet-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:placeholder-gray-500"
        }`}
      />
      <svg
        className={`pointer-events-none absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 ${value ? "text-violet-500 dark:text-violet-400" : "text-gray-400"}`}
        fill="none"
        viewBox="0 0 24 24"
        strokeWidth={1.8}
        stroke="currentColor"
      ...
      {value && (
        <button
          type="button"
          onClick={() => on_change("")}
          className="absolute right-2.5 top-1/2 -translate-y-1/2 text-violet-400 hover:text-violet-600 dark:hover:text-violet-300"
        >
...
function StatsStrip({ orders, total_from_api }: StatsStripProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 rounded-xl border border-violet-100 bg-violet-50/60 px-4 py-3 dark:border-violet-500/20 dark:bg-violet-500/5">
...
function getTotalPages(order: AdminOrder): number {
  return order.items.reduce((sum, item) => {
    const co_count = item.co_intake_rows?.length ?? 0;
    return sum + (co_count > 0 ? co_count : item.quantity);
  }, 0);
}

// ── Main component ─────────────────────────────────────────────────────────────

export default function AdminContentOptimizationOrdersContent() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    setError(null);
    try {
      const filters: AdminContentOptimizationOrderFilters = {
        page: current_page,
        per_page: 15,
        sort_field,
        sort_direction,
      ...
      const result = await listAdminContentOptimizationOrders(filters);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="mb-1.5 inline-flex items-center gap-2 rounded-lg border border-violet-200 bg-violet-50 px-3 py-1 dark:border-violet-500/30 dark:bg-violet-500/10">
            <span className="h-1.5 w-1.5 rounded-full bg-violet-500" />
            <span className="text-xs font-semibold uppercase tracking-wide text-violet-700 dark:text-violet-300">
              Content Optimization
            </span>
          </div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Content Optimization Orders</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Review client content optimization orders and their intake form data.
          </p>
        </div>
        <Link
          href="/admin/content-optimization"
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-gray-200 bg-white pl-10 pr-4 text-sm text-gray-900 placeholder-gray-400 transition-colors focus:border-violet-400 focus:outline-none focus:ring-3 focus:ring-violet-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white dark:placeholder-gray-500"
            />
          </div>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {is_loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-5 py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-full bg-violet-50 dark:bg-violet-500/10">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                          {has_active_filters ? "Try adjusting your search or filters." : "Content optimization orders will appear here once placed."}
                        </p>
                      </div>
                      {has_active_filters && (
                        <button
                          onClick={clearFilters}
                          className="mt-1 text-xs font-medium text-violet-600 hover:text-violet-700 dark:text-violet-400 dark:hover:text-violet-300"
                        >
                          Clear all filters
                        </button>
                      )}
                ...
                orders.map((order) => {
                  ...
                  const total_pages = getTotalPages(order);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div>
                          <p className="font-medium text-gray-900 transition-colors group-hover:text-violet-600 dark:text-white dark:group-hover:text-violet-400">
                            {order.order_title || "—"}
                          </p>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2.5">
                          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-violet-100 text-xs font-bold text-violet-700 dark:bg-violet-500/20 dark:text-violet-400">
                      ...
                      <td className="px-5 py-4 text-center">
                        <span className="inline-flex items-center justify-center rounded-full border border-violet-200 bg-violet-50 px-2.5 py-0.5 text-xs font-semibold text-violet-700 dark:border-violet-500/30 dark:bg-violet-500/10 dark:text-violet-300">
                          {total_pages}
                        </span>
                      </td>
