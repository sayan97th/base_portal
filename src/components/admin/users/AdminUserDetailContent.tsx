...
function getOrderStatusConfig(status: OrderStatus): {
  ...
  switch (status) {
    case "new_request":
    case "pending":
      return { color: "info", dot: "bg-teal-500", label: "New Request" };
    case "processing":
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 space-y-4 lg:col-span-4">
          ...
          {/* Additional Info — only rendered when at least one field has a value */}
          {(user.company || user.google_studio_link || user.referrer_id || user.note) && (
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
              <div className="flex items-center gap-2 border-b border-gray-200 px-5 py-4 dark:border-gray-800">
                <span className="text-gray-400 dark:text-gray-500">
                  <UserIcon />
              ...
              <dl className="px-5 py-1">
                ...
                {user.google_studio_link && (
                  <InfoRow
                    label="Google Studio"
                    icon={<LinkIcon />}
                    value={
                      <a
                        href={user.google_studio_link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-1 text-brand-600 hover:underline dark:text-brand-400"
                      >
                ...
                {user.note && (
                  <div className="border-b border-gray-100 py-2.5 last:border-b-0 dark:border-gray-800">
                    <dt className="mb-1.5 flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400">
                      <span className="text-gray-400 dark:text-gray-500">
                        <NoteIcon />
                      </span>
                      Note
                    </dt>
                    <dd className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-words">
                      {user.note}
                    </dd>
                  </div>
                )}
