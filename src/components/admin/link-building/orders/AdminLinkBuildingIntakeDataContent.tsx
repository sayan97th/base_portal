...
import LinkBuildingIntakeEditor, {
  type EditorItem,
} from "@/components/link-building/orders/LinkBuildingIntakeEditor";

interface AdminLinkBuildingIntakeDataContentProps {
  order_id: string;
}
...
export default function AdminLinkBuildingIntakeDataContent({
  order_id,
}: AdminLinkBuildingIntakeDataContentProps) {
  ...
  const editor_items = useMemo<EditorItem[]>(() => {
    if (!order) return [];
    return order.items
      .filter((item) => (item.placements?.length ?? 0) > 0)
      .map((item) => ({
        id: String(item.id),
        label: item.dr_tier?.label ?? item.item_name ?? "Link Building",
  ...
  return (
    <LinkBuildingIntakeEditor
      ...
      status={order.status}
      items={editor_items}
      onSave={(placements) => adminOrderDetailsService.submitLinkBuilding(order.id, placements)}
      back_href={`/admin/orders/${order.id}`}
      is_admin
    />
  );
}
