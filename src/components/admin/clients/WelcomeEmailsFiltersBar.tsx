...
import type { ClientSortField, PasswordResetStatusFilter, SortDirection } from "@/types/admin";
...
// ── Password reset status options ──────────────────────────────────────────────

interface ResetStatusOption {
  value: PasswordResetStatusFilter;
  label: string;
  dot_class: string;
  active_class: string;
}

const RESET_STATUS_OPTIONS: ResetStatusOption[] = [
  {
    value: "",
    label: "All",
...
interface WelcomeEmailsFiltersBarProps {
  search_value: string;
  on_search_change: (value: string) => void;
  password_reset_status: PasswordResetStatusFilter;
  on_password_reset_status_change: (status: PasswordResetStatusFilter) => void;
...
export default function WelcomeEmailsFiltersBar({
  search_value,
  on_search_change,
  password_reset_status,
  on_password_reset_status_change,
  sort_field,
  sort_direction,
  on_sort_change,
  date_from,
  ...
  is_loading,
  on_clear_all,
}: WelcomeEmailsFiltersBarProps) {
  const has_search = search_value.length > 0;
  const has_reset_status = password_reset_status !== "";
  const has_date = date_from.length > 0 || date_to.length > 0;
  const has_sort = sort_field !== undefined;
  const has_active_filters = has_search || has_reset_status || has_date || has_sort;
  const active_filter_count = [has_search, has_reset_status, has_date, has_sort].filter(Boolean).length;
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xs dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="flex items-center gap-3 border-b border-gray-100 px-4 py-3 dark:border-gray-800">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2.5 pl-9 pr-9 text-sm text-gray-900 placeholder-gray-400 outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-2 focus:ring-indigo-400/20 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500 dark:focus:border-indigo-400 dark:focus:bg-gray-800"
          />
          {has_search && (
            <button
        ...
        {has_active_filters && (
          <div className="flex shrink-0 items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400">
      ...
      {/* ── Row 2: Password reset status pills ── */}
      <div className="flex items-center gap-x-4 border-b border-gray-100 px-4 py-3 dark:border-gray-800">
        <div className="flex flex-wrap items-center gap-1.5">
          <div className="mr-1 flex shrink-0 items-center gap-1">
            <svg className="h-3.5 w-3.5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z" />
            </svg>
            <span className="text-xs font-medium text-gray-400 dark:text-gray-500">Password Reset</span>
          </div>
          {RESET_STATUS_OPTIONS.map((opt) => {
            const is_active = password_reset_status === opt.value;
            return (
              <button
                key={opt.value || "all-reset"}
                onClick={() => on_password_reset_status_change(opt.value)}
            ...
            return (
              <button
                ...
                className={`group inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-all duration-150 ${
                  is_active
                    ? "bg-indigo-500 text-white shadow-sm shadow-indigo-500/25 dark:bg-indigo-400 dark:text-gray-900"
                    : "bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"
                }`}
              >
