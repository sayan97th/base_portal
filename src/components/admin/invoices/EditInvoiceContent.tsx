  ...
  return (
    <label className={`flex items-start gap-3 ${disabled ? "opacity-50" : "cursor-pointer"}`}>
      <button
        ...
        onClick={() => !disabled && on_change(!checked)}
        disabled={disabled}
        className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border-2 transition-colors focus:outline-none ${
          disabled
            ...
            : checked
            ? "border-brand-500 bg-brand-500 focus:ring-2 focus:ring-brand-300 focus:ring-offset-1 dark:border-brand-400 dark:bg-brand-500"
            : "border-gray-300 bg-white focus:ring-2 focus:ring-brand-300 focus:ring-offset-1 dark:border-gray-600 dark:bg-gray-800"
        }`}
      >
        {!disabled && checked && (
        ...
        {disabled && (
          <svg className="h-3 w-3 text-gray-400 dark:text-gray-600" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
...
function EditInvoiceSkeletonLoader({ invoice_id }: { invoice_id: string }) {
  return (
    <div className="space-y-6">
      <div>
        <Link
          href={`/admin/invoices/${invoice_id}`}
...
export default function EditInvoiceContent({ invoice_id }: { invoice_id: string }) {
  ...
  const [send_update_notification, setSendUpdateNotification] = useState(true);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setLoadError(api_error?.message || "Failed to load invoice. Please try again.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <EditInvoiceSkeletonLoader invoice_id={invoice_id} />;
  }

  if (load_error || !invoice) {
    return (
      <div className="space-y-6">
        <Link
          href={`/admin/invoices/${invoice_id}`}
      ...
      <div>
        ...
        <div className="mt-4">
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">
            Edit Invoice{" "}
            <span className="text-gray-400 dark:text-gray-500">#{invoice.invoice_number}</span>
          </h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Update line items, due date, and notes for this invoice
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="space-y-4 lg:col-span-4">
          ...
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div className="space-y-4 p-5">
              <div>
                ...
                {/* Render DatePickerField once data is loaded, using invoice_id as key to ensure correct defaultDate */}
                <DatePickerField
          ...
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div className="space-y-4 p-5">
              <StyledCheckbox
                checked={send_update_notification}
                on_change={setSendUpdateNotification}
                label="Notify client of updates"
                description="Send an email to the client informing them that this invoice has been updated"
              />
              <div className="border-t border-gray-100 pt-4 dark:border-gray-800">
                <StyledCheckbox
                  checked={false}
                  on_change={() => {}}
                  label="Notify client (new invoice)"
                  description="Not available when editing an existing invoice"
                  disabled
                />
              </div>
              <StyledCheckbox
          ...
          <p className="text-center text-xs text-gray-400 dark:text-gray-500">
            Changes will be reflected immediately in the client&apos;s portal
          </p>
        </div>
      </div>
    </form>
