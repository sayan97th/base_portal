...
import type {
  ...
  AdminScheduledCallFilters,
  AdminScheduledCallStatus,
  AdminScheduledCallSortField,
  SortDirection,
...
const SORTABLE_COLS: { field: AdminScheduledCallSortField; label: string }[] = [
...
function SortIcon({
  ...
  field: AdminScheduledCallSortField;
  active_field?: AdminScheduledCallSortField;
...
export default function AdminScheduledCallsContent() {
  const [appointments, setAppointments] = useState<AdminScheduledCallAppointment[]>([]);
  ...
  const [status_filter, setStatusFilter] = useState<AdminScheduledCallStatus | "">("");
  const [sort_field, setSortField] = useState<AdminScheduledCallSortField>("scheduled_at");
  ...
  const fetchAppointments = useCallback(async (filters: AdminScheduledCallFilters) => {
    setIsLoading(true);
    setError(null);
    try {
      const data = await adminScheduledCallService.fetchAppointments(filters);
  ...
  function handleStatusFilter(value: AdminScheduledCallStatus | "") {
    setStatusFilter(value);
    setPage(1);
  }

  function handleColumnSort(field: AdminScheduledCallSortField) {
    if (sort_field === field) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {total > 0
              ? `${total} total scheduled calls`
              : "Manage all scheduled call requests from clients"}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          value={status_filter}
          onChange={(e) => handleStatusFilter(e.target.value as AdminScheduledCallStatus | "")}
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
                        ...
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-xs text-gray-400 dark:text-gray-500">
                            {has_active_filters
                              ...
                              : "Scheduled calls will appear here once clients submit requests."}
                          </p>
                          {has_active_filters && (
                            <button
                              onClick={handleClearFilters}
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-4">
                          <Link href={`/admin/scheduled-calls/${apt.id}`} className="block">
                        ...
                        <td className="px-5 py-4">
                          <Link href={`/admin/scheduled-calls/${apt.id}`} className="block">
                            <span
                              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ${status_cfg.badge}`}
                            >
                        ...
                        <td className="px-5 py-4 text-right">
                          <Link
                            href={`/admin/scheduled-calls/${apt.id}`}
        ...
        {!is_loading && last_page > 1 && (
          <div className="flex items-center justify-between border-t border-gray-200 px-5 py-3 dark:border-gray-800">
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Page {page} of {last_page} &middot; {total} calls
            </p>
