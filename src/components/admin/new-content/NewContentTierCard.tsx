...
import type { AdminNewContentTier } from "@/types/admin/new-content";

interface NewContentTierCardProps {
  tier: AdminNewContentTier;
  onEdit: () => void;
  onDelete: () => void;
  onToggleStatus: (is_active: boolean) => void;
}

function StatusBadge({ tier }: { tier: AdminNewContentTier }) {
  if (!tier.is_active) {
    return (
...
export default function NewContentTierCard({
  tier,
  onEdit,
  onDelete,
  onToggleStatus,
}: NewContentTierCardProps) {
  const price_num = typeof tier.price === "string" ? parseFloat(tier.price) : tier.price;
      ...
      <div className="flex flex-1 flex-col p-5">
          ...
          <div className="flex items-center gap-3">
            <div
              className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${
                tier.is_active ? "bg-brand-50 dark:bg-brand-500/10" : "bg-gray-100 dark:bg-gray-800"
              }`}
            >
              <svg
                className={`h-5 w-5 ${tier.is_active ? "text-brand-500" : "text-gray-400"}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1.8}
        ...
        <div className="mt-4 flex items-end gap-1">
          <span className="text-3xl font-bold text-gray-900 dark:text-white">
            ${isNaN(price_num) ? "—" : price_num.toFixed(0)}
          </span>
          <span className="mb-1 text-sm text-gray-400 dark:text-gray-500">/ piece</span>
        </div>
        ...
        <div className="mt-4 space-y-2 rounded-xl border border-gray-100 p-3 dark:border-gray-800">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-500 dark:text-gray-400">
              Turnaround:{" "}
              <span className="font-medium text-gray-700 dark:text-gray-300">{tier.turnaround_time}</span>
            </span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <svg className="h-4 w-4 shrink-0 text-brand-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
