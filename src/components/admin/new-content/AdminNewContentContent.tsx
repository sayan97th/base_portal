...
import {
  listAdminNewContentTiers,
  createAdminNewContentTier,
  updateAdminNewContentTier,
  toggleAdminNewContentTierStatus,
  deleteAdminNewContentTier,
} from "@/services/admin/new-content.service";
import NewContentTierCard from "./NewContentTierCard";
import NewContentTierFormModal from "./NewContentTierFormModal";
...
export default function AdminNewContentContent() {
  ...
  const [editing_tier, setEditingTier] = useState<AdminNewContentTier | null>(null);

  const [deleting_tier, setDeletingTier] = useState<AdminNewContentTier | null>(null);
  ...
  const fetchTiers = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await listAdminNewContentTiers();
      setTiers(data);
    } catch {
      setError("Failed to load content tiers. Please try again.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const filtered_tiers = tiers
    .filter((t) => {
      ...
      const matches_search =
        ...
        t.turnaround_time.toLowerCase().includes(q);
  ...
  const handleCreate = async (payload: CreateNewContentTierPayload) => {
    await createAdminNewContentTier(payload);
    fetchTiers();
  };

  const handleUpdate = async (payload: CreateNewContentTierPayload) => {
    if (!editing_tier) return;
    await updateAdminNewContentTier(editing_tier.id, payload as UpdateNewContentTierPayload);
    fetchTiers();
  };

  const handleToggleStatus = async (tier: AdminNewContentTier, is_active: boolean) => {
    await toggleAdminNewContentTierStatus(tier.id, is_active);
    fetchTiers();
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteAdminNewContentTier(deleting_tier.id);
  ...
  const openEdit = (tier: AdminNewContentTier) => {
    setEditingTier(tier);
    setFormModalOpen(true);
  };
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="h-7 w-56 animate-pulse rounded-lg bg-gray-200 dark:bg-gray-800" />
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by label, ID, or turnaround time..."
