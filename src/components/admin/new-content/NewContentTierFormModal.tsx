...
interface NewContentTierFormModalProps {
  is_open: boolean;
  tier: AdminNewContentTier | null;
  onClose: () => void;
  onSubmit: (payload: CreateNewContentTierPayload) => Promise<void>;
}

const EMPTY_FORM: CreateNewContentTierPayload = {
  id: "",
  label: "",
  turnaround_time: "",
  price: 0,
...
export default function NewContentTierFormModal({
  is_open,
  tier,
  onClose,
  onSubmit,
}: NewContentTierFormModalProps) {
  const [form_data, setFormData] = useState<CreateNewContentTierPayload>(EMPTY_FORM);
  ...
  const setField = <K extends keyof CreateNewContentTierPayload>(
    key: K,
    value: CreateNewContentTierPayload[K]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form_data.turnaround_time.trim()) {
      setError("Turnaround time is required.");
      return;
    }
        ...
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-5 dark:border-gray-800">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
              {is_editing ? "Edit Content Tier" : "Add Content Tier"}
            </h2>
            <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
              {is_editing
                ? "Update the details of this New Content tier."
                : "Create a new tier for the New Content service."}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              disabled={is_editing}
              className="h-11 w-full rounded-lg border border-gray-300 px-4 py-2.5 font-mono text-sm text-gray-900 placeholder-gray-400 transition-colors focus:border-brand-500 focus:outline-none focus:ring-3 focus:ring-brand-500/10 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500 dark:disabled:bg-gray-800/50"
            />
            {is_editing && (
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
              Turnaround Time <span className="text-error-500">*</span>
            </label>
            <input
              type="text"
              value={form_data.turnaround_time}
              onChange={(e) => setField("turnaround_time", e.target.value)}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Limit the number of units a client can order
                </p>
              </div>
              <button
                type="button"
                ...
                className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors ${
                  has_max_quantity ? "bg-brand-500" : "bg-gray-300 dark:bg-gray-600"
                }`}
              >
                <span
                  className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${
            ...
            {has_max_quantity && (
              <div className="mt-3">
                <input
                  ...
                  placeholder="e.g. 10"
                  className="h-10 w-full rounded-lg border border-gray-300 px-4 text-sm text-gray-900 transition-colors focus:border-brand-500 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                />
              </div>
            )}
          </div>
