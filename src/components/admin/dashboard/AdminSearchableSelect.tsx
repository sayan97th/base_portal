...
import type { AdminUserOption } from "@/services/admin/link-building-dashboard.service";
...
export default function AdminSearchableSelect({
  ...
  const content = (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-between border-b border-gray-100 bg-indigo-50 px-3 py-2.5 dark:border-gray-700 dark:bg-indigo-900/20">
        <div className="min-w-0 flex-1">
          ...
          {selected_user ? (
            <p className="mt-0.5 flex items-center gap-1 truncate text-[10px] text-indigo-600 dark:text-indigo-400">
              <span className="inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-indigo-500" />
              {selected_user.name}
            </p>
          ) : (
            <p className="mt-0.5 text-[10px] text-indigo-500/60 dark:text-indigo-500/50">
        ...
        <button
          onClick={onClose}
          className="ml-2 shrink-0 rounded-lg p-1 text-indigo-500 transition-colors hover:bg-indigo-100 hover:text-indigo-700 dark:hover:bg-indigo-800/50 dark:hover:text-indigo-300"
          title="Close"
        >
      ...
      <div className="border-b border-gray-100 p-2 dark:border-gray-700">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border border-gray-200 bg-gray-50 py-1.5 pl-8 pr-8 text-xs outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 dark:placeholder-gray-500 dark:focus:ring-indigo-900/40"
          />
          {search && (
            <button
              onClick={() => {
          ...
          return (
            ...
            >
              {user.avatar_url ? (
                ...
                <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-[11px] font-bold text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300">
            ...
            {search && (
              <>
                ...
                <button
                  ...
                  className="mt-2 rounded-lg px-3 py-1 text-[10px] font-medium text-indigo-600 transition-colors hover:bg-indigo-50 dark:text-indigo-400 dark:hover:bg-indigo-900/30"
                >
                  Clear search
                </button>
              </>
      ...
      <div className="flex items-center justify-between border-t border-gray-100 px-3 py-2 dark:border-gray-700">
        <p className="text-[10px] text-gray-400 dark:text-gray-500">
          {search.trim()
            ? `${filtered_users.length} of ${admin_users.length} member${admin_users.length !== 1 ? "s" : ""}`
            : `${admin_users.length} team member${admin_users.length !== 1 ? "s" : ""} total`}
        </p>
...
function highlightMatch(text: string, query: string): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="rounded bg-indigo-100 px-0.5 text-indigo-800 dark:bg-indigo-900/60 dark:text-indigo-200">
