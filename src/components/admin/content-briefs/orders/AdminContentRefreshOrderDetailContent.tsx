...
import { getAdminContentRefreshOrder } from "@/services/admin/content-refresh.service";
import type {
  AdminOrder,
  AdminInvoice,
  OrderItem,
...
interface AdminContentRefreshOrderDetailContentProps {
  order_id: string;
}
...
function getStatusConfig(status: OrderStatus): {
  ...
  switch (status) {
    ...
    case "processing": return { color: "info", label: "Processing", dot: "bg-amber-500" };
...
const OrderItemsTable = ({ items, coupons, total_amount }: OrderItemsTableProps) => {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-amber-200 bg-white dark:border-amber-500/30 dark:bg-white/3">
      {/* Header strip */}
      <div className="flex items-center gap-3 border-b border-amber-200 bg-amber-50 px-5 py-2.5 dark:border-amber-500/30 dark:bg-amber-500/10">
        <span className="inline-flex items-center rounded-md border border-amber-200 bg-amber-50 px-2.5 py-0.5 text-xs font-semibold text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
          Content Refresh
        </span>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-800/50">
              ...
              <th className="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Refresh Tier</th>
            ...
            {items.map((item, index) => (
              <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                ...
                <td className="px-5 py-3.5">
                  <span className="inline-flex items-center rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
            ...
            {(has_bulk || has_coupons) ? (
              <>
                <tr className="border-t border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/50">
                  <td colSpan={4} className="px-5 py-3 text-right text-sm text-gray-500 dark:text-gray-400">Subtotal</td>
                ...
                {has_bulk && (
                  <tr className="bg-amber-50/40 dark:bg-amber-500/5">
                    <td colSpan={4} className="px-5 py-2.5 text-right text-xs font-medium text-amber-700 dark:text-amber-300">
                      Bulk Discount (10% off)
                    </td>
                    <td className="px-5 py-2.5 text-right text-sm font-semibold tabular-nums text-amber-600 dark:text-amber-400">
                ...
                <tr className="border-t border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/50">
                  <td colSpan={4} className="px-5 py-3.5 text-right text-sm font-semibold text-gray-700 dark:text-gray-300">Total</td>
...
export default function AdminContentRefreshOrderDetailContent({
  order_id,
}: AdminContentRefreshOrderDetailContentProps) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setError(null);
      try {
        const data = await getAdminContentRefreshOrder(order_id);
        setOrder(data);
      } catch {
  ...
  return (
    <div className="space-y-6">
      {/* Back link */}
      <Link
        href="/admin/content-refresh/orders"
      ...
      >
        ...
        Back to Content Refresh Orders
      </Link>
      ...
      {!is_loading && order && status_config && (
        <>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="mb-2 inline-flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-1.5 dark:border-amber-500/30 dark:bg-amber-500/10">
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/3">
                ...
                <div className="px-5 py-4">
                  <div className="flex items-center gap-4">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-100 text-sm font-bold text-amber-700 dark:bg-amber-500/20 dark:text-amber-400">
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/3">
                ...
                <dl className="px-5 py-1">
                  <InfoRow
                    label="Product"
                    value={
                      <span className="inline-flex items-center rounded-md border border-amber-200 bg-amber-50 px-2 py-0.5 text-xs font-semibold text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
                        Content Refresh
                      </span>
                    }
                  />
                  <InfoRow label="Status" value={
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium capitalize ${
                      ...
                      order.status === "processing" ? "bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400" :
