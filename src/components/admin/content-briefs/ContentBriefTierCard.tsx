...
import type { AdminContentBriefTier } from "@/types/admin/content-briefs";

interface ContentBriefTierCardProps {
  tier: AdminContentBriefTier;
  onEdit: () => void;
  onDelete: () => void;
  onToggleStatus: (is_active: boolean) => void;
}

function StatusBadge({ tier }: { tier: AdminContentBriefTier }) {
  if (!tier.is_active) {
    return (
...
function TurnaroundLabel({ days }: { days: number }) {
  if (days === 1) return <>{days} business day</>;
  return <>{days} business days</>;
}

export default function ContentBriefTierCard({
  tier,
  onEdit,
  onDelete,
  onToggleStatus,
}: ContentBriefTierCardProps) {
  return (
    <div
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-4 flex items-end gap-1">
          ...
          <span className="mb-1 text-sm text-gray-400 dark:text-gray-500">/ brief</span>
        </div>
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-500 dark:text-gray-400">
              ...
              <span className="font-medium text-gray-700 dark:text-gray-300">
                <TurnaroundLabel days={tier.turnaround_days} />
              </span>
            </span>
          </div>
          <div className="flex items-center gap-2 text-sm">
