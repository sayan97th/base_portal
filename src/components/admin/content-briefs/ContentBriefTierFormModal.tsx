...
interface ContentBriefTierFormModalProps {
  is_open: boolean;
  tier: AdminContentBriefTier | null;
  onClose: () => void;
  onSubmit: (payload: CreateContentBriefTierPayload) => Promise<void>;
}

const EMPTY_FORM: CreateContentBriefTierPayload = {
  id: "",
  label: "",
  turnaround_days: 5,
  price: 0,
...
export default function ContentBriefTierFormModal({
  is_open,
  tier,
  onClose,
  onSubmit,
}: ContentBriefTierFormModalProps) {
  const [form_data, setFormData] = useState<CreateContentBriefTierPayload>(EMPTY_FORM);
  ...
  const setField = <K extends keyof CreateContentBriefTierPayload>(
    key: K,
    value: CreateContentBriefTierPayload[K]
        ...
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-5 dark:border-gray-800">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
              {is_editing ? "Edit Content Brief Tier" : "Add Content Brief Tier"}
            </h2>
            <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
              {is_editing
                ? "Update the details of this Content Brief tier."
                : "Create a new tier for the Content Briefs service."}
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Limit the number of briefs a client can order
                </p>
              </div>
              <button
                type="button"
