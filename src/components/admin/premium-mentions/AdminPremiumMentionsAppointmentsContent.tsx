...
import { adminPremiumMentionsAppointmentService } from "@/services/admin/premium-mentions.service";
import type {
  AdminPremiumMentionsAppointment,
  AdminPremiumMentionsAppointmentFilters,
  PremiumMentionsAppointmentStatus,
  PremiumMentionsAppointmentSortField,
  PremiumMentionsAppointmentSortDirection,
...
const SORTABLE_COLS: { field: PremiumMentionsAppointmentSortField; label: string }[] = [
...
function SortIcon({
  ...
  field: PremiumMentionsAppointmentSortField;
  active_field?: PremiumMentionsAppointmentSortField;
  direction: PremiumMentionsAppointmentSortDirection;
}) {
...
export default function AdminPremiumMentionsAppointmentsContent() {
  const [appointments, setAppointments] = useState<AdminPremiumMentionsAppointment[]>([]);
  ...
  const [status_filter, setStatusFilter] = useState<PremiumMentionsAppointmentStatus | "">("");
  const [sort_field, setSortField] = useState<PremiumMentionsAppointmentSortField>("scheduled_at");
  const [sort_direction, setSortDirection] = useState<PremiumMentionsAppointmentSortDirection>("desc");
  const [date_from, setDateFrom] = useState("");
  const [date_to, setDateTo] = useState("");

  const debounced_search = useDebounce(search_input, 450);

  const fetchAppointments = useCallback(async (filters: AdminPremiumMentionsAppointmentFilters) => {
    setIsLoading(true);
    setError(null);
    try {
      const data = await adminPremiumMentionsAppointmentService.fetchAppointments(filters);
  ...
  }, [fetchAppointments, page, debounced_search, status_filter, sort_field, sort_direction, date_from, date_to]);
  ...
  function handleStatusFilter(value: PremiumMentionsAppointmentStatus | "") {
    setStatusFilter(value);
    setPage(1);
  }

  function handleColumnSort(field: PremiumMentionsAppointmentSortField) {
    if (sort_field === field) {
  ...
  const has_active_filters =
    !!search_input || !!status_filter || !!date_from || !!date_to;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">
            Premium Mentions Appointments
          </h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {total > 0
              ? `${total} total appointments`
              : "Manage all Premium Mentions consultation appointments"}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          value={status_filter}
          onChange={(e) => handleStatusFilter(e.target.value as PremiumMentionsAppointmentStatus | "")}
                        ...
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-xs text-gray-400 dark:text-gray-500">
                            {has_active_filters
                              ...
                              : "Appointments will appear here once clients schedule Premium Mentions sessions."}
                          </p>
                          {has_active_filters && (
                            <button
                              onClick={handleClearFilters}
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-4">
                          <Link href={`/admin/premium-mentions/appointments/${apt.id}`} className="block">
                        ...
                        <td className="px-5 py-4">
                          <Link href={`/admin/premium-mentions/appointments/${apt.id}`} className="block">
                            {apt.plan ? (
                              <span className="inline-flex rounded-full bg-violet-50 px-2.5 py-0.5 text-xs font-medium text-violet-700 dark:bg-violet-500/10 dark:text-violet-400">
                                {apt.plan.name}
                              </span>
                            ) : (
                              <span className="text-xs text-gray-400">{apt.plan_id}</span>
                            )}
                          </Link>
                        </td>
                        <td className="px-5 py-4">
                        ...
                        <td className="px-5 py-4 text-right">
                          <Link
                            href={`/admin/premium-mentions/appointments/${apt.id}`}
