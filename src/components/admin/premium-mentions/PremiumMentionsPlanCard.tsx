...
import type { AdminPremiumMentionsPlan } from "@/types/admin/premium-mentions";

interface PremiumMentionsPlanCardProps {
  plan: AdminPremiumMentionsPlan;
  onEdit: () => void;
  onDelete: () => void;
  onToggleStatus: (is_active: boolean) => void;
}

function PlanStatusBadge({ plan }: { plan: AdminPremiumMentionsPlan }) {
  if (!plan.is_active) {
    return (
...
function PlacementRow({ label, value }: { label: string; value: number }) {
  return (
...
export default function PremiumMentionsPlanCard({
  plan,
  onEdit,
  onDelete,
  onToggleStatus,
}: PremiumMentionsPlanCardProps) {
  return (
    <div
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
          ...
          <div className="flex items-center gap-3">
            <div
              className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${
                plan.is_active ? "bg-brand-50 dark:bg-brand-500/10" : "bg-gray-100 dark:bg-gray-800"
              }`}
            >
              <svg
                className={`h-5 w-5 ${plan.is_active ? "text-brand-500" : "text-gray-400"}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1.8}
        ...
        {/* Tagline */}
        {plan.tagline && (
          <p className="mt-3 text-xs text-gray-500 dark:text-gray-400 italic">&ldquo;{plan.tagline}&rdquo;</p>
        )}
        ...
        <div className="mt-4 space-y-1.5 rounded-xl border border-gray-100 p-3 dark:border-gray-800">
          ...
          <PlacementRow label="Total" value={plan.total_placements} />
          <PlacementRow label="Exclusive" value={plan.exclusive_placements} />
          <PlacementRow label="Core" value={plan.core_placements} />
          <PlacementRow label="Support" value={plan.support_placements} />
        </div>
        ...
        {plan.best_for && (
          <div className="mt-3 flex items-start gap-2 text-sm text-gray-500 dark:text-gray-400">
            ...
            <span>{plan.best_for}</span>
          </div>
        )}

        {/* Stats row */}
        {(plan.orders_count !== undefined || plan.revenue_total !== undefined) && (
          <div className="mt-4 flex items-center gap-4 border-t border-gray-100 pt-4 dark:border-gray-800">
            {plan.orders_count !== undefined && (
              <div className="flex items-center gap-1.5">
                <svg className="h-3.5 w-3.5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
                <span className="text-xs font-medium text-gray-600 dark:text-gray-400">
                  {plan.orders_count} order{plan.orders_count !== 1 ? "s" : ""}
                </span>
              </div>
            )}
            {plan.revenue_total !== undefined && (
              <div className="flex items-center gap-1.5">
                <svg className="h-3.5 w-3.5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <span className="text-xs font-medium text-gray-600 dark:text-gray-400">
                  {(() => {
                    const r = Number(plan.revenue_total);
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button
            onClick={() => onToggleStatus(!plan.is_active)}
