...
const STATUS_TRANSITIONS: Record<PremiumMentionsAppointmentStatus, PremiumMentionsAppointmentStatus[]> = {
...
function SkeletonLoader() {
  return (
    <div className="space-y-6">
      <Link
        href="/admin/premium-mentions/appointments"
...
interface UpdateModalProps {
  appointment: AdminPremiumMentionsAppointment;
  modal_status: PremiumMentionsAppointmentStatus | "";
  modal_scheduled_at: string;
  modal_admin_notes: string;
  is_saving: boolean;
  onStatusChange: (s: PremiumMentionsAppointmentStatus | "") => void;
...
interface AdminPremiumMentionsAppointmentDetailProps {
  appointment_id: number;
}

export default function AdminPremiumMentionsAppointmentDetail({
  appointment_id,
}: AdminPremiumMentionsAppointmentDetailProps) {
  const router = useRouter();
  const [appointment, setAppointment] = useState<AdminPremiumMentionsAppointment | null>(null);
  const [is_loading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [show_update_modal, setShowUpdateModal] = useState(false);
  const [modal_status, setModalStatus] = useState<PremiumMentionsAppointmentStatus | "">("");
  ...
  const fetchAppointment = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await adminPremiumMentionsAppointmentService.fetchAppointmentById(appointment_id);
      setAppointment(data);
    } catch {
  ...
  async function handleUpdateConfirm() {
    ...
    setIsSaving(true);
    try {
      const payload: UpdatePremiumMentionsAppointmentPayload = {};
      ...
      const updated = await adminPremiumMentionsAppointmentService.updateAppointment(
        appointment.id,
        payload
      );
  ...
  async function handleDeleteConfirm() {
    ...
    setIsDeleting(true);
    try {
      await adminPremiumMentionsAppointmentService.deleteAppointment(appointment.id);
      router.push("/admin/premium-mentions/appointments");
    } catch {
              ...
              <InfoRow
                ...
                value={
                  appointment.plan ? (
                    ...
                    <span className="font-mono text-xs text-gray-500">{appointment.plan_id}</span>
                  )
                }
              />
              {appointment.plan && (
                <InfoRow
                  label="Plan Price"
                  value={`$${Number(appointment.plan.price_per_month).toLocaleString("en-US", { minimumFractionDigits: 2 })}/mo`}
                />
              )}
              <InfoRow
                label="Scheduled At"
