...
import {
  listAdminPremiumMentionsPlans,
  createAdminPremiumMentionsPlan,
  updateAdminPremiumMentionsPlan,
  toggleAdminPremiumMentionsPlanStatus,
  deleteAdminPremiumMentionsPlan,
} from "@/services/admin/premium-mentions.service";
import PremiumMentionsPlanCard from "./PremiumMentionsPlanCard";
import PremiumMentionsPlanFormModal from "./PremiumMentionsPlanFormModal";
...
function DeleteConfirmModal({ plan, is_deleting, onConfirm, onCancel }: DeleteConfirmModalProps) {
  if (!plan) return null;
  return (
...
export default function AdminPremiumMentionsContent() {
  const [plans, setPlans] = useState<AdminPremiumMentionsPlan[]>([]);
  ...
  const [editing_plan, setEditingPlan] = useState<AdminPremiumMentionsPlan | null>(null);

  const [deleting_plan, setDeletingPlan] = useState<AdminPremiumMentionsPlan | null>(null);
  ...
  const fetchPlans = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await listAdminPremiumMentionsPlans();
      setPlans(data);
    } catch {
  ...
  const active_count = plans.filter((p) => p.is_active).length;
  const disabled_count = plans.filter((p) => !p.is_active).length;

  // ── Filter ─────────────────────────────────────────────────────────────────

  const filtered_plans = plans
    .filter((p) => {
      ...
      const matches_status =
        status_filter === "all" ||
        (status_filter === "active" && p.is_active) ||
        (status_filter === "disabled" && !p.is_active);
  ...
  const handleCreate = async (payload: CreatePremiumMentionsPlanPayload) => {
    await createAdminPremiumMentionsPlan(payload);
    fetchPlans();
  };

  const handleUpdate = async (payload: CreatePremiumMentionsPlanPayload) => {
    if (!editing_plan) return;
    await updateAdminPremiumMentionsPlan(editing_plan.id, payload as UpdatePremiumMentionsPlanPayload);
    fetchPlans();
  };

  const handleToggleStatus = async (plan: AdminPremiumMentionsPlan, is_active: boolean) => {
    await toggleAdminPremiumMentionsPlanStatus(plan.id, is_active);
    fetchPlans();
  };

  const handleDelete = async () => {
    if (!deleting_plan) return;
    setIsDeleting(true);
    try {
      await deleteAdminPremiumMentionsPlan(deleting_plan.id);
  ...
  const openEdit = (plan: AdminPremiumMentionsPlan) => {
    setEditingPlan(plan);
    setFormModalOpen(true);
  };
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Manage Premium Mentions plans, placements, and pricing.
            </p>
          </div>
          <button
            onClick={openAdd}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by name, tagline, or best for..."
        ...
        {filtered_plans.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 py-16 dark:border-gray-700">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {search_query || status_filter !== "all"
                ...
                : "Add your first Premium Mentions plan to get started."}
            </p>
            {!search_query && status_filter === "all" && (
              <button
                onClick={openAdd}
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {filtered_plans.map((plan) => (
              <PremiumMentionsPlanCard
                key={plan.id}
                plan={plan}
                onEdit={() => openEdit(plan)}
                onDelete={() => setDeletingPlan(plan)}
                onToggleStatus={(is_active) => handleToggleStatus(plan, is_active)}
              />
            ))}
          </div>
        )}
      ...
      {/* Add / Edit modal */}
      <PremiumMentionsPlanFormModal
        is_open={form_modal_open}
        plan={editing_plan}
        onClose={() => { setFormModalOpen(false); setEditingPlan(null); }}
        onSubmit={editing_plan ? handleUpdate : handleCreate}
      />
