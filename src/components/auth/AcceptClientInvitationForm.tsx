...
export default function AcceptClientInvitationForm({ token }: Props) {
  ...
  useEffect(() => {
    const validate = async () => {
      try {
        const result = await validateClientInvitationToken(token);
        if (result.valid) {
      ...
      <div className="flex flex-col justify-center flex-1 w-full max-w-md mx-auto py-10">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            You have been invited to join BASE Search Marketing as a client.
          </p>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Registering with:{" "}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              />
              {field_errors.first_name && (
                <p className="mt-1 text-xs text-error-500">{field_errors.first_name[0]}</p>
              )}
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
              ...
              />
              {field_errors.last_name && (
                <p className="mt-1 text-xs text-error-500">{field_errors.last_name[0]}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            {field_errors.password && (
              <p className="mt-1 text-xs text-error-500">{field_errors.password[0]}</p>
            )}
          </div>
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-lg bg-brand-600 py-2.5 text-sm font-medium text-white transition-colors hover:bg-brand-700 disabled:opacity-60 dark:bg-brand-500 dark:hover:bg-brand-600"
          >
            {is_submitting && (
              <span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" />
            )}
            {is_submitting ? "Creating account…" : "Create My Account"}
          </button>
        </form>
      </div>
    </div>
