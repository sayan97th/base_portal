...
export default function SignInForm() {
  ...
  const [error, setError] = useState(() => {
    const err_param = typeof window !== "undefined"
      ? new URLSearchParams(window.location.search).get("error")
      : null;
    return err_param ? (GOOGLE_ERROR_MESSAGES[err_param] ?? "An error occurred. Please try again.") : "";
  });
            ...
            <div>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-1 sm:gap-5">
                ...
                >
                  ...
                  {is_google_loading ? "Redirecting…" : "Sign in with Google"}
                </button>
              </div>
