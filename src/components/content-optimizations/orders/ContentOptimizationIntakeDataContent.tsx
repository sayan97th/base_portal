...
function exportIntakeToCsv(order: ContentOptimizationOrderDetail) {
  const rows: string[][] = [
  ...
  link.href = url;
  link.download = `co-keywords-${order.id.slice(0, 8).toUpperCase()}.csv`;
  link.click();
        ...
        <>
          ...
          {items_with_intake.length === 0 && (
            <div className="flex flex-col items-center gap-4 rounded-xl border border-gray-200 bg-white py-16 text-center dark:border-gray-800 dark:bg-white/3">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-900 dark:text-white">No keyword data</p>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                  This order does not have any keyword data attached yet.
                </p>
              </div>
              <Link
                href="/content-refresh/content-optimizations"
          ...
          {items_with_intake.length > 0 && (
            <div className="space-y-10">
              {items_with_intake.map((item, item_index) => (
                <IntakeSection
                  key={item.id}
                  tier_name={item.tier?.label ?? `Tier ${item_index + 1}`}
